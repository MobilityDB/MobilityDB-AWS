...
This repository serves as a guide for distributing a PostgreSQL database on Amazon Web Services' Elastic Kubernetes Service (EKS). The primary objective is to enhance the scalability of a moving object database, which is supported by the MobilityDB extension for PostgreSQL. The aim is to optimize performance. To achieve this, the Citus extension for PostgreSQL has been selected. It facilitates data partitioning and distributes queries across the cluster, contributing to improved performance and responsiveness.
